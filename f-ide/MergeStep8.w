...
; n,m,k := 0,0,0
; keeping
	n <= N and m <= M and k = n + m and sorted[prefix[result,k]] and permutation[prefix[result,k], append[prefix[xs,n], prefix[ys,m]]] and Max[prefix[result,k]] <= Min[append[suffix[xs,N-n], suffix[ys,M-m]]]
  do n != N and m != M ->	
		if xs[n] < ys[m] -> result[k],k,n := xs[n], k+1, n+1
		[] xs[n] >= ys[m] -> result[k],k,m := ys[m], k+1, m+1 
    fi
  [] n = N and m != M -> result[k],k,m := ys[m],k+1,m+1
  [] m = M and n != N -> result[k],k,n := xs[n],k+1,n+1
  od 
